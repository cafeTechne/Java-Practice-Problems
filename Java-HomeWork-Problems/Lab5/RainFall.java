...
public class RainFall {
	...
	/**
	 * Private array of doubles holding the total rainfall for each month.
	 */
	private double totalRainFallArray[] = new double [12];
	
	/**
	 * A constructor which accepts an array from the client class.
	 * @param arr An array of doubles representing the rainfall for each month of a given year.
	 */
	public RainFall(double arr[]) {
		this.settotalRainFallArray(arr);
	}
	...
	public double getYearlyTotalRainFall() {
		...
		for(int i = 0; i<this.gettotalRainFallArray().length;i++) {
			runningSum += this.gettotalRainFallArray()[i];
		}
		return runningSum;
	}
	...
	public int getMonthWithMostRain() {
		double mostRain = this.gettotalRainFallArray()[0];
		int monthIndex = 0;
		for(int i = 0; i<this.gettotalRainFallArray().length;i++) {
			
			if (this.gettotalRainFallArray()[i] > mostRain) {
				mostRain = this.gettotalRainFallArray()[i];
				monthIndex = i;
			}
		}
		return monthIndex+1;
	...
	public int getMonthWithLeastRain() {
		double leastRain = this.gettotalRainFallArray()[0];
		int monthIndex = 0;
		for(int i = 0; i<this.gettotalRainFallArray().length;i++) {
			
			if (this.gettotalRainFallArray()[i] < leastRain) {
				leastRain = this.gettotalRainFallArray()[i];
				monthIndex = i;
			}
		}
		return monthIndex+1;
	...
	public double getRainfallOfSpecificMonth(int monthIndex) {
		return this.gettotalRainFallArray()[monthIndex - 1];
	}
	...
	/**
	 * An accessor method to get the total rain fall array. (optional)
	 * @return A reference to an array of doubles representing the total rain fall for the year.
	 */

	public double[] gettotalRainFallArray() {
		return totalRainFallArray;
	}

	
	/**
	 * A mutator method to assign an array of doubles representing the total rain fall for the year.
	 * @param totalRainFallArray An array of doubles representing the total rain fall for the year.
	 */

	public void settotalRainFallArray(double totalRainFallArray[]) {
		this.totalRainFallArray = totalRainFallArray;
	}
	...
	public void getAboveAverageMonths() {
		int numOfMonths = 0;
		for(int i=0; i< this.gettotalRainFallArray().length; i++) {
			if(this.gettotalRainFallArray()[i] > this.getAverageMonthlyRainFall()) {
				numOfMonths++;
			}	
		}
		...
		for(int i=0; i< this.gettotalRainFallArray().length; i++) {
			if(this.gettotalRainFallArray()[i] > this.getAverageMonthlyRainFall()) {
				System.out.println(this.getMonthName(i+1) + "    " + this.gettotalRainFallArray()[i]);
			}	
		}
	}
